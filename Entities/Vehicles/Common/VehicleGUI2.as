...
void drawChargeBar(CBlob@ blob, VehicleInfo@ v)
{
	...
	Vec2f dim = Vec2f(20, 8);
	const f32 y = blob.getHeight() * 2.4f;
	f32 last_charge_percent = v.getCurrentAmmo().max_charge_time / float(v.getCurrentAmmo().max_charge_time);
...
void drawCooldownBar(CBlob@ blob, VehicleInfo@ v)
{
	if (v.cooldown_time > 0)
	{
		...
		f32 modified_last_charge_percent = float(v.getCurrentAmmo().max_charge_time) / float(v.last_charge);
...
void drawLastFireCharge(CBlob@ blob, VehicleInfo@ v)
{
	...
	f32 range = (3 / float(v.getCurrentAmmo().max_charge_time));
