...
void onInit(CBlob@ this)
{
	...
	AddIconToken("$fighter_defend$", "Orders.png", Vec2f(32,32), 1);
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	else if (cmd == this.getCommandID("switch_class"))
	{
		...
		if (isServer())
		{
			CBlob@ b = CreateMigrant(this.getPosition(), 1, "archer");
			if (b !is null)
			{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (!dontHitMore)
				{
					...
					this.server_Hit(b, hi.hitpos, velocity, damage * this.get_f32("weakness"), type, true);  // server_Hit() is server-side only
