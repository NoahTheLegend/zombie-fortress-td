...
{
    if (team is null) @team = getRules().getTeam(0);
	...
	GUI::DrawPane(topleft, bottomright, 0xccff4000);
	...
	GUI::DrawText("Defender",   Vec2f(topleft.x, topleft.y), SColor(0xffffffff));
    GUI::DrawText("Owned",      Vec2f(topleft.x+200, topleft.y), SColor(0xffffffff));
    GUI::DrawText("Defender",   Vec2f(topleft.x, topleft.y), SColor(0xffffffff));
	GUI::DrawText("Pseudonym",  Vec2f(bottomright.x - 470 - offsetx, topleft.y), SColor(0xffffffff));
	GUI::DrawText("Echo",       Vec2f(bottomright.x - 330 - offsetx, topleft.y), SColor(0xffffffff));
	GUI::DrawText("Slayed",     Vec2f(bottomright.x - 260 - offsetx, topleft.y), SColor(0xffffffff));
	GUI::DrawText("Fallen",     Vec2f(bottomright.x - 190 - offsetx, topleft.y), SColor(0xffffffff));
	GUI::DrawText("Facilitated",Vec2f(bottomright.x - 130 - offsetx, topleft.y), SColor(0xffffffff));
	GUI::DrawText("Efficiency", Vec2f(bottomright.x - 70, topleft.y), SColor(0xffffffff));
	if(draw_age)
	{
	...
	for (u32 i = 0; i < players.length; i++)
	{
        ...
        Vec2f ownpos = topleft + Vec2f(name_buffer + 135 + clantag_actualsize.x + 8, 0);
        GUI::DrawText(p.getCoins()+" D", ownpos, 0xffffaa00);

        string[] filenames;
        u16[]    fileframes;
        Vec2f[]  filesizes;
        u16[]    quantities;
        if (p.getBlob() !is null && p.getBlob().get("scoreboard_items", filenames)
            && p.getBlob().get("scoreboard_item_sizes", filesizes)
            && p.getBlob().get("scoreboard_item_frames", fileframes)
            && p.getBlob().get("scoreboard_item_quantities", quantities))
        {
            if (filenames.size() == filesizes.size()
                && filenames.size() == fileframes.size()
                && filenames.size() == quantities.size())
            {
                for (u8 i = 0; i < filenames.size(); i++)
                {
                    Vec2f npos = ownpos + Vec2f(60 + i*30, -7);
                    GUI::DrawIcon(filenames[i], fileframes[i], filesizes[i], npos, 0.66f, p.getTeamNum());
                    GUI::SetFont("default");
                    GUI::DrawTextCentered(""+quantities[i], npos+Vec2f(16,20), SColor(255,255,255,255));
                    GUI::SetFont("menu");
                }
            }
        }
		...
		GUI::DrawText("" + username, Vec2f(bottomright.x - 470 - offsetx, topleft.y), namecolour);
		GUI::DrawText("" + ping_in_ms, Vec2f(bottomright.x - 330 - offsetx, topleft.y), SColor(0xffffffff));
